...
object Folding {
  ...
  object ListExercises {
    ...
    def flatMap[A, B](list: List[A])(f: A => List[B]): List[B] = {
      list.foldLeft(List.empty[B])((concurrentList, a) => concurrentList.foldRight(f(a))(_ :: _))
    }
  ...
  val combined = (Foldable[List] compose Foldable[Vector]).combineAll(intsNested)
  ...
  def main(args: Array[String]): Unit = {
    ...
    println(s"flatMap((1 to 10).toList)() = ${flatMap(numbers)(a => (1 to a).toList)}")

    println(filter(numbers)(_ % 2 == 0))

    println(combineAll(numbers))

    println(s"sumOption = ${sumOption}")
