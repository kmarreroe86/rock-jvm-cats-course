...
object Readers {
  ...
  val dbReader: Reader[Configuration, DbConnection] = Reader(conf => DbConnection(conf.dbUsername, conf.dbPassword))  // Reads from Configuration and will create a DbConnection
  val dbConn: DbConnection = dbReader.run(config) // Return a DbConnection

  // Reader[I, O]
  val orderStatusReader: Reader[Configuration, String] = dbReader.map(dbConn => dbConn.getOrderStatus(1L))
...
//    userLastOrderIdReader.run(config)
  ...
  def main(args: Array[String]): Unit = {
    ...
    println(s"getLastOrderStatus(\"karel\") = ${getLastOrderStatus("karel")}")
...
// https://rockthejvm.com/courses/1107955/lectures/23728904